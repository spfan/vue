...
TemplateRenderer.prototype.renderScripts = function renderScripts (context) {
  ...
  if(!this.options.inlineScripts) {
    ...
    // 内联脚本
    if (this.clientManifest) {
      var initial$1 = this.preloadFiles.filter(function (ref) {
          var file = ref.file;

          return isJS(file);
        });
      var async$1 = (this.getUsedAsyncFiles(context) || []).filter(function (ref) {
          var file = ref.file;

          return isJS(file);
        });
      var needed$1 = [initial$1[0]].concat(async$1, initial$1.slice(1));
      return needed$1.map(function (ref) {
          var file = ref.file;

        var fileCode = this$1.options.files[file];
        // 公共chunk
...
function createRenderer (ref) {
  ...
  var files = ref.files;
  var inlineScripts = ref.inlineScripts; if ( inlineScripts === void 0 ) inlineScripts = false;
  ...
  var templateRenderer = new TemplateRenderer({
    ...
    files: files, inlineScripts: inlineScripts  // 扩展的变量
  });
